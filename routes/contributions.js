...
router.get('/pendingContributions', async (req, res) => {  
    if (req.session.AuthCookie === undefined) {
      res.render("login/notAuthorized", { isDisplayLogout: false });
        ...
        if(userdetails.userType.trim() == "admin"){
            ...
            res.render("./contributions/pendingContri", { isDisplayLogout: true, contributions: contriArray, title: "pending reqs" }); 
        }
       }
  });
